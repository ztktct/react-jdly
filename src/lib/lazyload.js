...
class Lazyload{
	...
	_loadAllImage(){
		...
		for(let i = 0, j = imglists.length;i<j;i++){
			...
			this._imgPosition(imglists[i], img =>{
				// 图片加载中
				if(typeof img.getAttribute(opts.state.loading) == 'object'){
	...
	init(){
		...
		_.on( wrap, 'scroll', _self._loadAllImage.bind(_self));

		_.on( wrap, 'resize', _self._loadAllImage.bind(_self));
	}
	// 重新加载
	refresh(){
		this._loadAllImage();
	...
	destory(){
		let _self = this;
		let wrap = _self.wrap;
		_.off( wrap, 'scroll', _self._loadAllImage.bind(_self));
		_.off( wrap, 'resize', _self._loadAllImage.bind(_self));
